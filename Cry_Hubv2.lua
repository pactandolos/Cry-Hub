...
local t1 = {
	...
	Status = "preview",
	Game = "Steal an Egg",
	Discord = "https://discord.gg/2q9JwE5Nkj",
	Website = "",
	Changelog = "",
	Author = "Wicho (@2fa.)",
	Credits = "Nigga",
...
do
    ...
    do
        ...
        do
            ...
            t2 = {
    Dark = {
        bg          = Color3.fromRGB(14, 8, 8),      -- fondo principal (rojo muy oscuro)
        rail        = Color3.fromRGB(20, 10, 10),     -- barra lateral
        card        = Color3.fromRGB(38, 16, 16),     -- tarjetas
        lift        = Color3.fromRGB(52, 22, 22),     -- hover / elevación
        fill        = Color3.fromRGB(64, 26, 26),     -- relleno de controles
        line        = Color3.fromRGB(92, 36, 36),     -- bordes / líneas
        text        = Color3.fromRGB(255, 235, 235),  -- texto principal
        dim         = Color3.fromRGB(210, 160, 160),  -- texto secundario
        mute        = Color3.fromRGB(150, 100, 100),  -- texto apagado
        accent      = Color3.fromRGB(230, 60, 60),    -- acento rojo brillante
        accentDeep  = Color3.fromRGB(120, 20, 20),    -- acento oscuro
        accentHover = Color3.fromRGB(255, 90, 90),    -- acento hover
        ink         = Color3.fromRGB(30, 10, 10),     -- tinta (contraste)
        ok          = Color3.fromRGB(180, 90, 90),    -- estado "ok"
        Kira        = Color3.fromRGB(255, 235, 235)   -- color del logo
    },
    Light = {
        bg          = Color3.fromRGB(240, 225, 225),  -- fondo claro
        rail        = Color3.fromRGB(240, 225, 225),
        card        = Color3.fromRGB(255, 245, 245),
        lift        = Color3.fromRGB(250, 230, 230),
        fill        = Color3.fromRGB(235, 210, 210),
        line        = Color3.fromRGB(210, 180, 180),
        text        = Color3.fromRGB(40, 15, 15),
        dim         = Color3.fromRGB(110, 60, 60),
        mute        = Color3.fromRGB(150, 100, 100),
        accent      = Color3.fromRGB(200, 50, 50),
        accentDeep  = Color3.fromRGB(140, 30, 30),
        accentHover = Color3.fromRGB(220, 70, 70),
        ink         = Color3.fromRGB(255, 245, 245),
        ok          = Color3.fromRGB(120, 150, 100),  -- dejé el verde para "ok"
        Kira        = Color3.fromRGB(40, 15, 15)
    }
}
            t3 = {}
    ...
    ["Cosmic"]          = 700000000,
    ["Cherry Blossom"]  = 2500000000000,
    ["Titan Temple"]    = 7000000000000,
    ["Angels & Demons"] = 20000000000000
                        }
            t8 = {
				About = "info",
				["Auto Steal"] = "egg",
...
v281(v295, "AutoSpeedFilter", "Auto filter by speed", "solo roba huevos de biomas donde tu velocidad alcance (2x el mínimo)", true)
...
-- ============================================================
-- CRY HUB · Funciones de velocidad
-- ============================================================

-- Lee la velocidad actual del jugador desde leaderstats
local function vVelocidadActual()
    local ok, valor = pcall(function()
        local ls = LocalPlayer:FindFirstChild("leaderstats")
        if not ls then return 0 end
        local sp = ls:FindFirstChild("Speed")
            or ls:FindFirstChild("WalkSpeed")
            or ls:FindFirstChild("Velocidad")
            or ls:FindFirstChild("SpeedValue")
        if not sp then return 0 end
        return tonumber(sp.Value) or 0
    end)
...
-- Devuelve true si el bioma es seguro para tu velocidad actual
-- Regla: necesitas el DOBLE del mínimo para ir seguro
local function vBiomaSegunVelocidad(areaId)
    local vel = vVelocidadActual()
    if vel <= 0 then
        return true -- si no podemos leer la velocidad, no bloqueamos nada
    end
    local minimo = t300[tostring(areaId or "")]
    if not minimo or minimo == 0 then
        return true
    end
    return vel >= (minimo * 2)
end

-- Formatea la velocidad para mostrarla bonita
local function vFormatearVelocidad(v)
...
local function v1215(p308, p309, p310)
    ...
    -- NUEVO: si el toggle está activo, comprobar la velocidad primero
    if t9.AutoSpeedFilter then
        if not vBiomaSegunVelocidad(str24) then
            return false
        end
    end
    ...
    local velTxt = ""
    if vel > 0 then
        velTxt = " · " .. vFormatearVelocidad(vel) .. " spd"
    end
    AutoSteal.status.Text = string.format("%s · took %d · lost %d · re-grabbed %d%s", s32, t216.banked, t216.lost, t216.regrabs, velTxt)
end)
...
-- ============================================================
-- CRY HUB · Auto-ajuste de Areas según velocidad
-- ============================================================
task.spawn(function()
    task.wait(3) -- esperar a que todo cargue
    while u1117 do
        task.wait(5)
        if not u1117 then break end
        if t9.AutoSpeedFilter then
            pcall(function()
                local vel = vVelocidadActual()
                if vel <= 0 then return end

                -- Calcular qué biomas puede farmear (regla del doble)
                local permitidos = {}
                for _, bioma in ipairs(t5) do
                    local minimo = t300[bioma] or 0
                    if vel >= (minimo * 2) then
                        permitidos[#permitidos + 1] = bioma
                    end
                end

                -- Aplicar la nueva lista si cambió
                local actual = t9.Areas
                local cambio = false
                if type(actual) ~= "table" or #actual ~= #permitidos then
                    cambio = true
                else
                    for i = 1, #permitidos do
                        if actual[i] ~= permitidos[i] then
                            cambio = true
                            break
                        end
                    end
                ...
                if cambio then
                    t9.Areas = permitidos
                    local w = t10.Areas
                    if w and w.set then pcall(w.set, permitidos) end
                    if w and w.refresh then pcall(w.refresh) end
                    v1102("speed", "areas auto → " .. table.concat(permitidos, ", "))
                end
            end)
        end
    end
